...
_this spawn {
	...
	switch (_operation) do {
		...
		case "autoGuideOrdnance": {
			...
			//Get ordnance type
			switch (_callingObject getVariable ["INC_ammoType","missile"]) do {
				...
				case "bomb": {
					...
					_args params [["_launchPos",[0,0,15000]],["_primaryTarget",objNull],["_primaryLaunch",false],["_aircraftObject",sentinel]];
					...
					if (_primaryLaunch) then {
						_hqObject globalChat format ["%1: Rifle rifle rifle. Weapon away. Time of flight, %2 seconds.",_airCallsign, (round _travelTime)];
					...
					if (!(_callingObject getVariable ["INC_abortStrike",false]) && {_primaryLaunch}) then {_hqObject globalChat format ["%1: Splash.",_airCallsign]};
					...
					if (_primaryLaunch) then {
						[_callingObject] spawn {
							...
							_callingObject setVariable ["INC_strikeCompleted",true];
				...
				case "missile": {
					...
					_args params ["_launchPos","_primaryTarget",["_primaryLaunch",false],["_aircraftObject",sentinel]];
					...
					if (_primaryLaunch) then {
						...
						_hqObject globalChat format ["%1: Rifle rifle rifle, missile off the rail. Time of flight, %2 seconds.",_airCallsign, (round _travelEstimate)];
					};
		...
		case "strikeAftermath": {
			...
			_args params [["_ammoExpended",1],["_aircraftObject",sentinel]];

			private ["_strikeType"];

			_strikeType = _callingObject getVariable ["INC_ammoType","missile"];

			sleep (random 2);

			_ammoAvailable = [_callingObject,"SetAmmo",[_aircraftObject,_strikeType,_ammoExpended]] call SEN_fnc_senMain;
			...
			if (!_ammoAvailable) exitWith {
				...
				if (_rtbOnNoAmmo) then {
					...
					if (!isNil "sentinelAbort") then {deleteVehicle sentinelAbort};
					...
					missionNamespace setVariable ["INC_airMissionComplete", true, true]; //Prevent further missions
					missionNamespace setVariable ["INC_sentinelTracking", false, true]; // Turns off the signal tracking
				} else {
			...
			_ammoArray = missionNamespace getVariable ["SEN_ammoArray",[_bomb,_missile]];
